...
CREATE TABLE `employee_branches` (
    ...
    UNIQUE INDEX `employee_branches_user_id_branch_id_key`(`user_id`, `branch_id`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `shipments` (
    ...
    `qr_code_image` VARCHAR(191) NOT NULL,
    `price` DOUBLE NULL,
...
-- AddForeignKey
ALTER TABLE `shipment_histories` ADD CONSTRAINT `shipment_histories_user_id_fkey` FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `shipment_histories` ADD CONSTRAINT `shipment_histories_branch_id_fkey` FOREIGN KEY (`branch_id`) REFERENCES `branches`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
